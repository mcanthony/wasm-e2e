...
static unsigned int f2b(float value) {
  // min and max currently not supported.
...
class Vec3 {
 public:
  ...
  float nlDot(const Vec3& other) const {
    ...
    // Prevent LLVM from transforming this into a max.
    if (value < 0.00000000001f) {
      value = 0.0f;
    }
    return value;
  }
...
void sampleEnv(const Vec3& dir, Vec3* out) {
...
int intersect(const Vec3& pos, const Vec3& dir, Vec3* normal) {
  // The sphere.
